...
class Trie:
    ...
    def search_similar(self, word_) -> []:
        ...
        заполняется стек вызовов.
        Это O(number) вызовов функции, где number - число узлов (Так как в худшем случае мы обойдем все узлы в дереве).
        Для проверки каждого узла требуется 3 строки матрицы (предпредыдущая, предыдущая и текущая), также есть строка
        prefix, в которую добавляются символы при проходе по дереву. ее максимальная длина length+1. Остальные параметры
        имеют константный размер. length - длина проверяемого слова. Это O(length) по памяти для каждой функции.
